...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional(pd.DataFrame):
        ...
        ********************************************************************************************
        ...
        """
        try:
            threshold = sself.data_validation_config.missing_threshold
    ...
    def is_required_columns_exists(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:
        try:
            ...
            if len(missing_columns)>0:
                logging.info(f"Column:[{base} is not availabel.]")
            ...
            for base_column in base_columns:
                ...
                if same_disrtibution.pvalue>0.05:
                    drift_report[base_column]={
                        "pvalues":float(same_disrtibution.pvalue),
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df.repalce({"na":np.NAN},inplace=true)
            logging.info(f"Replace na value from base df")
            ...
            logging.inf(f"Reading test dataframe")
            ...
            if test_df_column_status:
                logging.info(f"As all column are avilable in test df hence detecting data drift")
